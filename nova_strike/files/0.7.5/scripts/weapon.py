...
class Weapon:
    ...
    def __init__(
        ...
        self.uses_view_model = bool(getattr(self.owner, "is_local_controlled", False))
        self.view_model = None
        if self.uses_view_model:
    ...
    def trigger_pull(self) -> bool:
        if not self.active:
            return False
        if hasattr(self.game_manager, "can_player_fire") and (not self.game_manager.can_player_fire(self.owner)):
            return False
        ...
        if hit.hit:
            ...
            if hasattr(hit.entity, "take_damage"):
                ...
                if self.rng.random() < crit_chance:
                    ...
                    self.game_manager.ui_manager.show_hitmarker()
                is_headshot = self._is_headshot_target(hit.entity, hit.world_point)
                if is_headshot:
                    scaled_damage *= self.HEADSHOT_DAMAGE_MULTIPLIER
                    self.game_manager.ui_manager.show_hitmarker()
                if hasattr(self.game_manager, "is_player_entity") and self.game_manager.is_player_entity(hit.entity):
                    if hasattr(self.game_manager, "request_player_damage"):
                        did_hit_enemy = bool(
                            self.game_manager.request_player_damage(
                                attacker=self.owner,
                                target=hit.entity,
                    ...
                    if did_hit_enemy and is_headshot:
                        self.game_manager.ui_manager.show_headshot_indicator()
                else:
                    hit.entity.take_damage(scaled_damage, self.owner)
                    self.game_manager.on_player_hit_enemy(hit.entity, hit.world_point, is_headshot=is_headshot)
    ...
    def _is_headshot_target(self, target, hit_point) -> bool:
        if not target or hit_point is None:
            return False
        if hasattr(target, "is_headshot"):
            try:
                return bool(target.is_headshot(hit_point))
            except Exception:
                return False
        try:
            target_world_y = float(getattr(target, "world_y"))
            half_height = float(getattr(target, "collider_half_height", 0.0))
            hit_y = float(hit_point.y)
        except Exception:
            return False
        if half_height <= 0.0:
            return False
        return hit_y >= (target_world_y + half_height * 0.66)
