...
class Player(Entity):
    ...
    def __init__(
        ...
        self.network_interp_delay = 0.1
        self.network_extrapolation_limit = 0.2
        self.network_last_timestamp = 0.0
        self.network_state_buffer = deque(maxlen=30)
    ...
    def take_damage(self, amount: float, _source=None, authoritative: bool = False) -> None:
        if not self.alive:
            return
        if (not authoritative) and (not self.is_local_controlled) and getattr(getattr(self.game_manager, "match_settings", None), "is_multiplayer", False):
            return
        if (not authoritative) and self.rng.random() < self.get_dodge_chance():
            if self.is_local_controlled:
                self.game_manager.ui_manager.show_toast("Dodged!", duration=0.45)
            return
        final_damage = float(amount)
        if not authoritative:
            final_damage *= self.get_incoming_damage_multiplier()
    ...
    def _update_remote_interpolation(self, dt: float) -> None:
        if self.network_state_buffer:
            now = float(getattr(self.game_manager, "network_clock", 0.0))
            render_time = max(0.0, now - float(self.network_interp_delay))
            while len(self.network_state_buffer) >= 2 and self.network_state_buffer[1]["recv_time"] <= render_time:
                self.network_state_buffer.popleft()
            if len(self.network_state_buffer) >= 2:
                older = self.network_state_buffer[0]
                newer = self.network_state_buffer[1]
                start_time = float(older["recv_time"])
                end_time = float(newer["recv_time"])
                span = max(1e-4, end_time - start_time)
                t = max(0.0, min(1.0, (render_time - start_time) / span))
                p0 = older["position"]
                p1 = newer["position"]
                self.network_target_position = p0 + ((p1 - p0) * t)
                self.network_target_velocity = older["velocity"] + ((newer["velocity"] - older["velocity"]) * t)
                self.network_target_rotation_y = float(older["rotation_y"]) + (
                    ((float(newer["rotation_y"]) - float(older["rotation_y"]) + 180.0) % 360.0) - 180.0
                ) * t
                self.network_target_pitch = float(older["pitch"]) + (float(newer["pitch"]) - float(older["pitch"])) * t
            else:
                latest = self.network_state_buffer[0]
                extrapolate = max(0.0, min(float(self.network_extrapolation_limit), render_time - float(latest["recv_time"])))
                self.network_target_position = latest["position"] + (latest["velocity"] * extrapolate)
                self.network_target_velocity = Vec3(latest["velocity"])
                self.network_target_rotation_y = float(latest["rotation_y"])
                self.network_target_pitch = float(latest["pitch"])
    ...
    def _handle_auto_fire(self) -> None:
        if hasattr(self.game_manager, "can_player_fire") and (not self.game_manager.can_player_fire(self)):
            return
        action_state = self._sample_action_state()
    ...
    def apply_network_state(self, state: PlayerSyncState) -> None:
        ...
        recv_time = float(getattr(self.game_manager, "network_clock", 0.0))
        snapshot = {
            "recv_time": recv_time,
            "position": Vec3(state.position.x, state.position.y, state.position.z),
            "velocity": Vec3(state.velocity.x, state.velocity.y, state.velocity.z),
            "rotation_y": float(state.rotation_y),
            "pitch": float(state.pitch),
        }
        self.network_state_buffer.append(snapshot)
        self.network_target_position = Vec3(snapshot["position"])
        self.network_target_velocity = Vec3(snapshot["velocity"])
        self.network_target_rotation_y = float(snapshot["rotation_y"])
        self.network_target_pitch = float(snapshot["pitch"])
    ...
    def respawn(self, position) -> None:
        ...
        self.regen_timer = self.regen_delay
        self._deactivate_active_ability()
        self.network_state_buffer.clear()
