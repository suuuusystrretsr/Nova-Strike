    ...
    def _simulate_step(self, dt: float) -> None:
        ...
        if hit.hit:
            ...
            if hasattr(target, "take_damage"):
                ...
                is_headshot = self._is_headshot_target(target, hit.world_point)
                if is_headshot:
                    final_damage *= self.HEADSHOT_DAMAGE_MULTIPLIER
                dealt = False
                if hasattr(self.game_manager, "is_player_entity") and self.game_manager.is_player_entity(target):
                    ...
                    if self.owner == local_player:
                        self.game_manager.on_player_hit_enemy(target, hit.world_point, is_headshot=is_headshot)
                ...
                if dealt and is_headshot and hasattr(self.game_manager.ui_manager, "show_headshot_indicator"):
                    local_player = self.game_manager.get_local_player() if hasattr(self.game_manager, "get_local_player") else None
                    if self.owner == local_player:
