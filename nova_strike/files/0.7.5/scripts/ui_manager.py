...
from ursina import Button, Entity, Text, camera, color, destroy, held_keys, invoke, lerp, mouse
...
class UIManager:
    ...
    def __init__(self, game_manager) -> None:
        ...
        self.headshot_timer = 0.0
        self.kill_feed_lines = []
    ...
    def _build_hud(self) -> None:
        ...
        self.headshot_text = Text(
            parent=self.hud,
            text="HEADSHOT",
            origin=(0, 0),
            position=(0, -0.08),
            scale=1.05,
            color=color.rgba(255, 210, 120, 0),
        )
        ...
        self.match_timer_text = Text(parent=self.hud, text="", position=(-0.11, 0.455), scale=0.6, color=color.rgb(222, 236, 252))
        self.ping_text = Text(parent=self.hud, text="", position=(0.59, 0.29), scale=0.47, color=color.rgb(182, 214, 242))
        self.lobby_status_text = Text(parent=self.hud, text="", position=(-0.34, 0.40), scale=0.46, color=color.rgb(190, 220, 250))

        self.kill_feed_root = Entity(parent=self.hud, position=(0.53, 0.12))
        self.kill_feed_texts = []
        for idx in range(5):
            line = Text(
                parent=self.kill_feed_root,
                text="",
                y=0.05 - (idx * 0.045),
                scale=0.42,
                color=color.rgb(232, 242, 255),
                enabled=False,
            )
            self.kill_feed_texts.append(line)
        ...
        self.scoreboard_panel = Entity(parent=self.root, enabled=False)
        Entity(parent=self.scoreboard_panel, model="quad", scale=(0.82, 0.56), color=color.rgba(4, 10, 18, 230), y=0.06)
        self.scoreboard_title = Text(parent=self.scoreboard_panel, text="SCOREBOARD", y=0.28, scale=0.8, color=color.rgb(210, 232, 255))
        self.scoreboard_body = Text(parent=self.scoreboard_panel, text="", x=-0.36, y=0.21, scale=0.52, color=color.rgb(196, 220, 245))
    ...
    def show_hud(self) -> None:
        ...
        self.scoreboard_panel.enabled = False
    ...
    def hide_hud(self) -> None:
        ...
        for entity in self.always_hud_entities:
            entity.enabled = False
        for line in self.kill_feed_texts:
            line.enabled = False
        self._hud_hidden_by_skill_tree = False
        self._state_paused_by_skill_tree = False
        self.close_skill_tree_panel()
    ...
    def update_hud(self, player) -> None:
        ...
        if self.headshot_timer > 0:
            self.headshot_timer -= self.game_manager.dt
            headshot_alpha = int(255 * clamp01(self.headshot_timer / 0.7))
            self.headshot_text.color = color.rgba(255, 212, 120, headshot_alpha)
        else:
            self.headshot_text.color = color.rgba(255, 212, 120, 0)

        self._update_multiplayer_overlay()
    ...
    def show_headshot_indicator(self) -> None:
        self.headshot_timer = 0.7
        self.headshot_text.color = color.rgba(255, 212, 120, 230)
        self.headshot_text.scale = 1.0
        self.headshot_text.animate_scale(1.18, duration=0.08)
        self.headshot_text.animate_scale(1.0, duration=0.08, delay=0.08)

    def add_kill_feed(self, message: str) -> None:
        text = str(message or "").strip()
        if not text:
            return
        self.kill_feed_lines.append({"text": text, "ttl": 8.0})
        if len(self.kill_feed_lines) > 8:
            self.kill_feed_lines = self.kill_feed_lines[-8:]
    ...
    def _update_multiplayer_overlay(self) -> None:
        snapshot = {}
        if hasattr(self.game_manager, "get_multiplayer_overlay_data"):
            snapshot = self.game_manager.get_multiplayer_overlay_data() or {}
        enabled = bool(snapshot.get("enabled", False))
        if not enabled:
            self.match_timer_text.text = ""
            self.ping_text.text = ""
            self.lobby_status_text.text = ""
            self.scoreboard_panel.enabled = False
            for line in self.kill_feed_texts:
                line.enabled = False
            return

        phase = str(snapshot.get("phase", "lobby") or "lobby").lower()
        elapsed = float(snapshot.get("elapsed", 0.0) or 0.0)
        duration = float(snapshot.get("duration", 0.0) or 0.0)
        countdown = float(snapshot.get("countdown", -1.0) or -1.0)
        ping_ms = int(snapshot.get("local_ping_ms", 0) or 0)
        connected = int(snapshot.get("connected_players", 0) or 0)
        ready = int(snapshot.get("ready_players", 0) or 0)
        required = int(snapshot.get("required_players", 2) or 2)

        if phase == "countdown":
            self.match_timer_text.text = f"START IN {max(0, int(math.ceil(countdown)))}"
            self.lobby_status_text.text = f"Lobby Ready {ready}/{max(required, connected)}"
        elif phase == "live":
            if duration > 0:
                remaining = max(0, int(duration - elapsed))
                mins = remaining // 60
                secs = remaining % 60
                self.match_timer_text.text = f"{mins:02d}:{secs:02d}"
            else:
                self.match_timer_text.text = "LIVE"
            self.lobby_status_text.text = "Match Live"
        elif phase == "ended":
            self.match_timer_text.text = "MATCH ENDED"
            self.lobby_status_text.text = "Press ESC for menu"
        else:
            self.match_timer_text.text = "LOBBY"
            self.lobby_status_text.text = f"Ready {ready}/{max(required, connected)}"
        self.ping_text.text = f"Ping: {max(0, int(ping_ms))}ms"

        # Merge network feed into local kill feed cache.
        for msg in snapshot.get("kill_feed", []):
            text = str(msg or "").strip()
            if not text:
                continue
            found = False
            for entry in self.kill_feed_lines:
                if entry.get("text") == text:
                    entry["ttl"] = max(float(entry.get("ttl", 0.0)), 4.0)
                    found = True
                    break
            if not found:
                self.kill_feed_lines.append({"text": text, "ttl": 6.5})
        updated_lines = []
        for entry in self.kill_feed_lines:
            ttl = float(entry.get("ttl", 0.0)) - float(self.game_manager.dt)
            if ttl <= 0:
                continue
            updated_lines.append({"text": str(entry.get("text", "")), "ttl": ttl})
        self.kill_feed_lines = updated_lines[-8:]

        for idx, line in enumerate(self.kill_feed_texts):
            if idx < len(self.kill_feed_lines[-5:]):
                entry = self.kill_feed_lines[-5:][idx]
                alpha = int(255 * clamp01(float(entry.get("ttl", 0.0)) / 6.5))
                line.text = str(entry.get("text", ""))
                line.color = color.rgba(232, 242, 255, alpha)
                line.enabled = True
            else:
                line.enabled = False

        show_scoreboard = bool(held_keys["tab"]) and self.hud.enabled and not self.map_open and not self.inventory_open and not self.skill_tree_open
        self.scoreboard_panel.enabled = show_scoreboard
        if show_scoreboard:
            rows = ["Player            K   D   HS   Ping   Team   Ready"]
            scoreboard = list(snapshot.get("scoreboard", []))
            scoreboard.sort(
                key=lambda row: (
                    -int(row.get("kills", 0)),
            ...
            for row in scoreboard[:16]:
                pid = str(row.get("player_id", ""))[:14]
                kills = int(row.get("kills", 0))
                deaths = int(row.get("deaths", 0))
                headshots = int(row.get("headshots", 0))
                ping = int(row.get("ping_ms", 0))
                team = str(row.get("team_id", "-") or "-")
                ready_flag = "Y" if bool(row.get("ready", False)) else "N"
                rows.append(f"{pid:<14} {kills:>2}  {deaths:>2}  {headshots:>2}  {ping:>4}   {team:<6} {ready_flag}")
            self.scoreboard_body.text = "\n".join(rows)
