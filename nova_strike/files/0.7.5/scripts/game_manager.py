...
class GameManager:
    ...
    def __init__(self, app) -> None:
        ...
        self.network_health_sync_timer = 0.0
        self.network_health_sync_interval = 0.25
        self.local_ping_ms = 0.0
        self.player_ping_ms: Dict[str, float] = {}
        self.pending_ping_requests: Dict[int, float] = {}
        self.next_ping_nonce = 1
        self.ping_interval = 1.0
        self.ping_timer = 0.0
        self.remote_authority_samples: Dict[str, dict] = {}
        self.player_position_history: Dict[str, deque] = {}

        self.match_phase = "inactive"
        self.match_elapsed = 0.0
        self.match_duration = 0.0
        self.match_countdown = -1.0
        self.lobby_ready_players: Dict[str, bool] = {}
        self.lobby_min_players = 2
        self.lobby_broadcast_timer = 0.0
        self.lobby_broadcast_interval = 0.35
        self.match_stats: Dict[str, Dict[str, int]] = {}
        self.kill_feed_events: list[dict] = []
        self.kill_feed_limit = 10
        self.disconnect_notice_cooldown = 0.0
        self.disconnect_notice_active = False
    ...
    def _is_multiplayer_match(self) -> bool:
        return bool(self.match_settings and self.match_settings.is_multiplayer)

    def _is_match_live(self) -> bool:
        if not self._is_multiplayer_match():
            return True
        return self.match_phase == "live"

    def can_player_fire(self, player: Optional[Player]) -> bool:
    ...
    def _resolve_match_duration_seconds(self) -> float:
        mode_id = str(getattr(self.match_settings, "mode_id", self.mode_id) or self.mode_id)
        if mode_id == "ctf":
            return 900.0
        if mode_id == "battle_royale":
            return 1500.0
        if mode_id == "duel_1v1":
            return 300.0
        return 600.0

    def _reset_multiplayer_runtime_state(self) -> None:
        self.match_phase = "inactive"
        self.match_elapsed = 0.0
        self.match_duration = 0.0
        self.match_countdown = -1.0
        self.lobby_ready_players = {}
        self.lobby_broadcast_timer = 0.0
        self.match_stats = {}
        self.kill_feed_events = []
        self.local_ping_ms = 0.0
        self.player_ping_ms = {}
        self.pending_ping_requests = {}
        self.next_ping_nonce = 1
        self.ping_timer = 0.0
        self.remote_authority_samples = {}
        self.player_position_history = {}
        self.network_health_sync_timer = 0.0
        self.disconnect_notice_cooldown = 0.0
        self.disconnect_notice_active = False

    def _initialize_multiplayer_runtime_state(self) -> None:
        self._reset_multiplayer_runtime_state()
        if not self._is_multiplayer_match():
            return
        self.match_duration = self._resolve_match_duration_seconds()
        self.match_phase = "lobby"
        self._register_player_for_match(self.local_player_id)
        self._set_player_ready_state(self.local_player_id, False, broadcast=False)
        self.ui_manager.show_toast("Lobby: press F1 to ready up", duration=1.8)

    def _register_player_for_match(self, player_id: str) -> None:
        pid = str(player_id or "").strip()
        if not pid:
            return
        if pid not in self.match_stats:
            self.match_stats[pid] = {"kills": 0, "deaths": 0, "headshots": 0}
        if pid not in self.lobby_ready_players:
            self.lobby_ready_players[pid] = False
        if pid not in self.player_ping_ms:
            self.player_ping_ms[pid] = 0.0
        if pid not in self.player_position_history:
            self.player_position_history[pid] = deque(maxlen=40)

    def _required_ready_count(self) -> int:
        if not self._is_multiplayer_match():
            return 1
        return max(2, int(getattr(self.match_settings, "min_players", 2)))

    def _active_multiplayer_player_ids(self) -> list[str]:
        player_ids = []
    ...
    def _set_player_ready_state(self, player_id: str, ready: bool, broadcast: bool = True) -> None:
        pid = str(player_id or "").strip()
        if not pid:
            return
        self._register_player_for_match(pid)
        self.lobby_ready_players[pid] = bool(ready)
        if broadcast and self.network_bridge.mode == self.network_bridge.MODE_SERVER:
            self._queue_lobby_state_packet(force=True)

    def _toggle_local_ready_state(self) -> None:
        if not self._is_multiplayer_match():
            return
        if self.match_phase not in ("lobby", "countdown"):
            return
        current = bool(self.lobby_ready_players.get(self.local_player_id, False))
        next_ready = not current
        self._set_player_ready_state(self.local_player_id, next_ready, broadcast=False)
        if self.network_bridge.mode == self.network_bridge.MODE_SERVER:
            ...
            self.network_bridge.queue_packet(
                {
                    "type": "lobby_ready",
                    "payload": {"player_id": self.local_player_id, "ready": bool(next_ready)},
                }
            )
        self.ui_manager.show_toast("Ready" if next_ready else "Not Ready", duration=1.0)

    def _all_required_players_ready(self) -> bool:
        active_ids = self._active_multiplayer_player_ids()
        if len(active_ids) < self._required_ready_count():
            return False
        for pid in active_ids:
            if not bool(self.lobby_ready_players.get(pid, False)):
                return False
        return True

    def _build_lobby_state_payload(self) -> dict:
        return {
            "mode_id": str(self.match_settings.mode_id if self.match_settings else self.mode_id),
            "map_id": str(self.current_map_id or ""),
            "phase": str(self.match_phase),
            "countdown": float(self.match_countdown),
            "elapsed": float(self.match_elapsed),
            "duration": float(self.match_duration),
            "ready": {str(pid): bool(ready) for pid, ready in self.lobby_ready_players.items()},
            "ping_ms": {str(pid): int(max(0.0, float(ms))) for pid, ms in self.player_ping_ms.items()},
        }

    def _queue_lobby_state_packet(self, force: bool = False) -> None:
        if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
            return
        if not self._is_multiplayer_match():
            return
        if not force and self.lobby_broadcast_timer > 0.0:
            return
        self.lobby_broadcast_timer = self.lobby_broadcast_interval
        self.network_bridge.queue_packet({"type": "lobby_state", "payload": self._build_lobby_state_payload()})

    def _handle_lobby_state_packet(self, payload: dict) -> None:
        if self.network_bridge.mode != self.network_bridge.MODE_CLIENT:
            return
        if not isinstance(payload, dict):
            return
        phase = str(payload.get("phase", self.match_phase) or self.match_phase)
        self.match_phase = phase
        try:
            self.match_countdown = float(payload.get("countdown", self.match_countdown))
        except (TypeError, ValueError):
            pass
        try:
            self.match_elapsed = max(0.0, float(payload.get("elapsed", self.match_elapsed)))
        except (TypeError, ValueError):
            pass
        try:
            self.match_duration = max(0.0, float(payload.get("duration", self.match_duration)))
        except (TypeError, ValueError):
            pass
        ready_payload = payload.get("ready", {})
        if isinstance(ready_payload, dict):
            self.lobby_ready_players = {str(pid): bool(ready) for pid, ready in ready_payload.items()}
        ping_payload = payload.get("ping_ms", {})
        if isinstance(ping_payload, dict):
            self.player_ping_ms = {}
            for pid, ping_value in ping_payload.items():
                try:
                    self.player_ping_ms[str(pid)] = max(0.0, float(ping_value))
                except (TypeError, ValueError):
                    self.player_ping_ms[str(pid)] = 0.0
            self.local_ping_ms = float(self.player_ping_ms.get(self.local_player_id, self.local_ping_ms))

    def _start_lobby_countdown(self) -> None:
        if self.match_phase != "lobby":
            return
        self.match_phase = "countdown"
        self.match_countdown = 5.0
        self.ui_manager.show_toast("All players ready. Match starts in 5...", duration=1.2)
        self._queue_lobby_state_packet(force=True)

    def _cancel_lobby_countdown(self) -> None:
        if self.match_phase != "countdown":
            return
        self.match_phase = "lobby"
        self.match_countdown = -1.0
        self.ui_manager.show_toast("Countdown canceled - waiting for all players ready", duration=1.1)
        self._queue_lobby_state_packet(force=True)

    def _start_live_phase(self) -> None:
        self.match_phase = "live"
        self.match_countdown = -1.0
        self.match_elapsed = 0.0
        self.ui_manager.show_toast("Match Started!", duration=1.3)
        if self.network_bridge.mode == self.network_bridge.MODE_SERVER:
            self.network_bridge.queue_packet({"type": "match_start", "payload": {"started_at": float(self.network_clock)}})
            self._queue_lobby_state_packet(force=True)

    def _end_live_phase(self) -> None:
        if self.match_phase == "ended":
            return
        self.match_phase = "ended"
        self.ui_manager.show_toast("Match ended", duration=1.4)
        if self.network_bridge.mode == self.network_bridge.MODE_SERVER:
            self.network_bridge.queue_packet({"type": "match_end", "payload": {"elapsed": float(self.match_elapsed)}})
            self._queue_lobby_state_packet(force=True)

    def _multiplayer_phase_update(self) -> None:
        if not self._is_multiplayer_match():
            return
        if self.network_bridge.mode == self.network_bridge.MODE_SERVER:
            self.lobby_broadcast_timer -= self.frame_dt
            if self.match_phase in ("lobby", "countdown"):
                if self._all_required_players_ready():
                    if self.match_phase == "lobby":
                        self._start_lobby_countdown()
                else:
                    self._cancel_lobby_countdown()
                if self.match_phase == "countdown":
                    self.match_countdown -= self.sim_dt
                    if self.match_countdown <= 0.0:
                        self._start_live_phase()
                self._queue_lobby_state_packet()
            elif self.match_phase == "live":
                self.match_elapsed += self.sim_dt
                if self.match_duration > 0.0 and self.match_elapsed >= self.match_duration:
                    self._end_live_phase()
                self._queue_lobby_state_packet()
            self.network_health_sync_timer -= self.frame_dt
            if self.network_health_sync_timer <= 0.0:
                self.network_health_sync_timer = self.network_health_sync_interval
                self._queue_health_sync_packet()
        elif self.match_phase == "live":
            self.match_elapsed += self.sim_dt

    def _queue_health_sync_packet(self) -> None:
        if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
            return
        payload = {"players": {}}
        for pid, player in self.players.items():
            if not player:
                continue
            payload["players"][str(pid)] = {
                "health": float(max(0.0, getattr(player, "health", 0.0))),
                "health_max": float(max(1.0, getattr(player, "health_max", 100.0))),
                "alive": bool(getattr(player, "alive", False)),
            }
        self.network_bridge.queue_packet({"type": "health_sync", "payload": payload})

    def _handle_health_sync_packet(self, payload: dict) -> None:
        if not isinstance(payload, dict):
            return
        players_payload = payload.get("players", {})
        if not isinstance(players_payload, dict):
            return
        for pid, stats in players_payload.items():
            if not isinstance(stats, dict):
                continue
            player = self.players.get(str(pid))
            if not player:
                continue
            try:
                player.health_max = max(1.0, float(stats.get("health_max", player.health_max)))
                player.health = max(0.0, min(player.health_max, float(stats.get("health", player.health))))
            except (TypeError, ValueError):
                pass
            alive = bool(stats.get("alive", player.alive))
            if alive and not player.alive:
                player.enable()
                player.alive = True
            elif (not alive) and player.alive:
                player.alive = False
                player.disable()

    def _queue_ping_packet(self) -> None:
        if self.network_bridge.mode != self.network_bridge.MODE_CLIENT:
            return
        if not self.network_bridge.connection_ready:
            return
        nonce = int(self.next_ping_nonce)
        self.next_ping_nonce += 1
        self.pending_ping_requests[nonce] = float(self.network_clock)
        self.network_bridge.queue_packet({"type": "ping", "payload": {"nonce": nonce, "t": float(self.network_clock)}})

    def _handle_ping_packet(self, payload: dict) -> None:
        if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
            return
        if not isinstance(payload, dict):
            return
        ...
        if nonce <= 0:
            return
        self.network_bridge.queue_packet({"type": "pong", "payload": {"nonce": nonce, "server_t": float(self.network_clock)}})

    def _handle_pong_packet(self, payload: dict) -> None:
        if self.network_bridge.mode != self.network_bridge.MODE_CLIENT:
            return
        if not isinstance(payload, dict):
            return
        try:
            ...
            return
        sent_at = self.pending_ping_requests.pop(nonce, None)
        if sent_at is None:
            return
        rtt = max(0.0, (float(self.network_clock) - float(sent_at)) * 1000.0)
        self.local_ping_ms = rtt
        self.player_ping_ms[self.local_player_id] = rtt
        self.network_bridge.queue_packet(
            {
                "type": "ping_report",
                "payload": {"player_id": self.local_player_id, "rtt_ms": float(rtt)},
            }
        )

    def _handle_ping_report_packet(self, payload: dict) -> None:
        if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
            return
        if not isinstance(payload, dict):
            return
        ...
        if not player_id:
            return
        try:
            ping_ms = max(0.0, min(999.0, float(payload.get("rtt_ms", 0.0))))
        except (TypeError, ValueError):
            return
        self.player_ping_ms[player_id] = ping_ms
        self._queue_lobby_state_packet(force=True)

    def _record_player_position_history(self, player_id: str, position: Vec3) -> None:
        pid = str(player_id or "")
        if not pid:
            return
        history = self.player_position_history.setdefault(pid, deque(maxlen=40))
        history.append({"t": float(self.network_clock), "position": Vec3(position)})

    def _validate_remote_movement(self, remote_state: PlayerSyncState) -> bool:
        if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
            return True
        player_id = str(remote_state.player_id or "")
        if not player_id:
            return False
        now = float(self.network_clock)
        sample = self.remote_authority_samples.get(player_id)
        if not sample:
            self.remote_authority_samples[player_id] = {
                "t": now,
                "position": Vec3(remote_state.position.x, remote_state.position.y, remote_state.position.z),
            }
            return True
        prev_t = float(sample.get("t", now))
        prev_pos = sample.get("position", Vec3(remote_state.position.x, remote_state.position.y, remote_state.position.z))
        dt = max(1.0 / 120.0, min(0.5, now - prev_t))
        incoming_pos = Vec3(remote_state.position.x, remote_state.position.y, remote_state.position.z)
        travel = (incoming_pos - prev_pos).length()
        max_speed = 24.0
        max_travel = (max_speed * dt) + 2.4
        if travel > max_travel:
            return False
        self.remote_authority_samples[player_id] = {"t": now, "position": incoming_pos}
        return True

    def is_player_entity(self, entity) -> bool:
        return isinstance(entity, Player)

    def _is_friendly_fire_blocked(self, attacker_id: str, target_id: str) -> bool:
        attacker_team = str(self.player_teams.get(str(attacker_id), "") or "")
        target_team = str(self.player_teams.get(str(target_id), "") or "")
        return bool(attacker_team and target_team and attacker_team == target_team)

    def request_player_damage(
        self,
        ...
        if not attacker or not target:
            return False
        if not self._is_multiplayer_match():
            target.take_damage(float(damage), attacker)
            return True
        if self.match_phase != "live":
            return False
        attacker_id = str(getattr(attacker, "player_id", "") or "")
        target_id = str(getattr(target, "player_id", "") or "")
        if not attacker_id or not target_id or attacker_id == target_id:
            return False
        if self._is_friendly_fire_blocked(attacker_id, target_id):
            return False
        if self.network_bridge.mode == self.network_bridge.MODE_SERVER:
            return self._apply_authoritative_player_damage(
        ...
        if self.network_bridge.mode == self.network_bridge.MODE_CLIENT and attacker_id == self.local_player_id:
            payload = {
                "target_id": target_id,
                "damage": float(max(1.0, min(300.0, float(damage)))),
                "is_headshot": bool(is_headshot),
                "weapon_id": str(weapon_id or ""),
                "shot_t": float(self.network_clock),
            }
            if hit_point is not None:
                try:
                    payload["hit_point"] = [float(hit_point.x), float(hit_point.y), float(hit_point.z)]
                except Exception:
                    pass
            self.network_bridge.queue_packet({"type": "damage_request", "payload": payload})
            return True
        return False

    def _schedule_player_respawn(self, player_id: str, delay: float = 3.0) -> None:
        pid = str(player_id or "")
        if not pid:
            return
        if self.mode_id == "battle_royale":
            return
        self.scheduler.schedule(max(0.2, float(delay)), self._respawn_player, pid, session_id=self.session_id)

    def _respawn_player(self, player_id: str) -> None:
        pid = str(player_id or "")
        player = self.players.get(pid)
        if not player or not self.world:
            return
        team_id = str(self.player_teams.get(pid, getattr(player, "team_id", "")) or "")
        spawn = self.world.get_player_spawn_point(team_id=team_id) if hasattr(self.world, "get_player_spawn_point") else self.world.safe_player_spawn
        try:
            player.respawn(spawn)
        except Exception:
            player.enable()
            player.alive = True
            player.health = float(player.health_max)
            ...
            self.network_bridge.queue_packet(
                {
                    ...
                    "payload": {
                        ...
                        "health_max": float(player.health_max),
                    },
                }
            )

    def _handle_respawn_packet(self, payload: dict) -> None:
        if not isinstance(payload, dict):
            return
        player_id = str(payload.get("player_id", "") or "")
        ...
        if isinstance(position, list) and len(position) == 3:
            try:
                respawn_pos = Vec3(float(position[0]), float(position[1]), float(position[2]))
            except (TypeError, ValueError):
                respawn_pos = None
        if respawn_pos is None:
            team_id = str(self.player_teams.get(player_id, getattr(player, "team_id", "")) or "")
            if self.world and hasattr(self.world, "get_player_spawn_point"):
                respawn_pos = self.world.get_player_spawn_point(team_id=team_id)
            elif self.world:
                respawn_pos = self.world.safe_player_spawn
        player.respawn(respawn_pos)
        try:
            player.health = max(1.0, float(payload.get("health", player.health_max)))
            player.health_max = max(1.0, float(payload.get("health_max", player.health_max)))
        except (TypeError, ValueError):
            pass

    def _apply_authoritative_player_damage(
        self,
        attacker_id: str,
        target_id: str,
        damage: float,
        ...
        if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
            return False
        attacker = self.players.get(str(attacker_id))
        target = self.players.get(str(target_id))
        if not attacker or not target:
            return False
        if not getattr(attacker, "alive", False) or not getattr(target, "alive", False):
            return False
        if self._is_friendly_fire_blocked(attacker_id, target_id):
            return False
        raw_damage = max(1.0, min(300.0, float(damage)))
        distance_between = (attacker.world_position - target.world_position).length()
        allowed_distance = 95.0
        if distance_between > allowed_distance:
            return False
        if hit_point is not None:
            try:
                if (Vec3(hit_point) - target.world_position).length() > 2.8:
                    return False
            except Exception:
                pass
        old_alive = bool(target.alive)
        target.take_damage(raw_damage, attacker, authoritative=True)
        final_damage = max(0.0, float(raw_damage))
        payload = {
            "attacker_id": str(attacker_id),
            "target_id": str(target_id),
            "damage": float(final_damage),
            "weapon_id": str(weapon_id or ""),
            "headshot": bool(is_headshot),
            "health": float(target.health),
            "health_max": float(target.health_max),
            "alive": bool(target.alive),
        }
        self.network_bridge.queue_packet({"type": "damage_event", "payload": payload})
        if old_alive and (not target.alive):
            self._register_player_for_match(attacker_id)
            self._register_player_for_match(target_id)
            self.match_stats[attacker_id]["kills"] += 1
            self.match_stats[target_id]["deaths"] += 1
            if bool(is_headshot):
                self.match_stats[attacker_id]["headshots"] += 1
            killer_label = "You" if attacker_id == self.local_player_id else attacker_id
            victim_label = "You" if target_id == self.local_player_id else target_id
            feed = f"{killer_label} eliminated {victim_label}{' (HEADSHOT)' if is_headshot else ''}"
            self.kill_feed_events.append({"time": float(self.network_clock), "message": feed})
            if len(self.kill_feed_events) > self.kill_feed_limit:
                self.kill_feed_events = self.kill_feed_events[-self.kill_feed_limit :]
            self.ui_manager.add_kill_feed(feed)
            self.network_bridge.queue_packet({"type": "kill_feed", "payload": {"message": feed}})
            self._schedule_player_respawn(target_id, delay=3.0)
        if bool(is_headshot) and attacker_id == self.local_player_id:
            self.ui_manager.show_headshot_indicator()
        if from_client:
            self._queue_lobby_state_packet(force=True)
        return True

    def _handle_damage_event_packet(self, payload: dict) -> None:
        if not isinstance(payload, dict):
            return
        ...
        if not player:
            return
        try:
            incoming_damage = float(payload.get("damage", 0.0))
        except (TypeError, ValueError):
            incoming_damage = 0.0
        if incoming_damage > 0.0 and player.alive:
            player.take_damage(incoming_damage, None, authoritative=True)
        try:
            player.health_max = max(1.0, float(payload.get("health_max", player.health_max)))
            player.health = max(0.0, min(player.health_max, float(payload.get("health", player.health))))
        except (TypeError, ValueError):
            pass
        alive = bool(payload.get("alive", player.alive))
        if alive and not player.alive:
            player.enable()
            player.alive = True
        elif (not alive) and player.alive:
    ...
    def _handle_kill_feed_packet(self, payload: dict) -> None:
        if not isinstance(payload, dict):
            return
        message = str(payload.get("message", "") or "").strip()
        if not message:
            return
        self.kill_feed_events.append({"time": float(self.network_clock), "message": message})
        if len(self.kill_feed_events) > self.kill_feed_limit:
            self.kill_feed_events = self.kill_feed_events[-self.kill_feed_limit :]
        self.ui_manager.add_kill_feed(message)

    def _resolve_attacker_id_from_packet(self, packet: dict) -> str:
        if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
            return ""
        peer = str(packet.get("_peer_addr", "") or "").strip()
        if not peer:
            return ""
        for addr, sock in list(self.network_bridge.clients.items()):
            label = f"{addr[0]}:{addr[1]}"
            if label != peer:
                continue
            return str(self.network_bridge.socket_player_ids.get(sock, "") or "")
        return ""

    def _handle_damage_request_packet(self, packet: dict, payload: dict) -> None:
        if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
            return
        if not isinstance(payload, dict):
            return
        attacker_id = self._resolve_attacker_id_from_packet(packet)
        if not attacker_id:
            return
        target_id = str(payload.get("target_id", "") or "")
        if not target_id:
        ...
        hit_point = payload.get("hit_point")
        world_point = None
        if isinstance(hit_point, list) and len(hit_point) == 3:
            try:
                world_point = Vec3(float(hit_point[0]), float(hit_point[1]), float(hit_point[2]))
            except (TypeError, ValueError):
                world_point = None
        self._apply_authoritative_player_damage(
            attacker_id=attacker_id,
            target_id=target_id,
            damage=damage,
    ...
    def get_multiplayer_overlay_data(self) -> dict:
        now = float(self.network_clock)
        self.kill_feed_events = [entry for entry in self.kill_feed_events if (now - float(entry.get("time", now))) <= 8.0]
        scoreboard = []
        for pid in sorted(self.match_stats.keys()):
            stats = self.match_stats.get(pid, {})
            scoreboard.append(
                {
                    "player_id": pid,
                    ...
                    "team_id": str(self.player_teams.get(pid, "")),
                    "ping_ms": int(max(0.0, float(self.player_ping_ms.get(pid, 0.0)))),
                    "ready": bool(self.lobby_ready_players.get(pid, False)),
                }
            )
        return {
            "enabled": bool(self._is_multiplayer_match()),
            "phase": str(self.match_phase),
            "countdown": float(self.match_countdown),
            "elapsed": float(self.match_elapsed),
            "duration": float(self.match_duration),
            "local_ping_ms": int(max(0.0, float(self.local_ping_ms))),
            "required_players": int(self._required_ready_count()),
            "connected_players": int(len(self._active_multiplayer_player_ids())),
            "ready_players": int(sum(1 for ready in self.lobby_ready_players.values() if ready)),
            "kill_feed": [str(entry.get("message", "")) for entry in self.kill_feed_events[-5:]],
    ...
    def start_gameplay(self) -> None:
        ...
        try:
            ...
            self.remote_last_seen_time = {}
            self._record_player_position_history(self.local_player_id, self.player.world_position)
            self.camera_controller = CameraController(self.player, self)
            self.camera_controller.update()
            if self._is_multiplayer_match():
                self._initialize_multiplayer_runtime_state()
                self._register_player_for_match(self.local_player_id)
                self.player_ping_ms[self.local_player_id] = self.local_ping_ms
            if self.network_bridge.mode == self.network_bridge.MODE_SERVER:
    ...
    def _update_multiplayer_prep_mode(self) -> None:
        # Keep local-only prompts disabled for competitive modes and drive lobby/match timers.
        self.ui_manager.set_interact_prompt("")
        self._multiplayer_phase_update()
    ...
    def on_player_hit_enemy(self, _enemy, _point, is_headshot: bool = False) -> None:
        if bool(is_headshot):
            self.ui_manager.show_headshot_indicator()
    ...
    def on_player_died(self) -> None:
        ...
        if self._is_multiplayer_match():
            if self.mode_id == "battle_royale":
                self.ui_manager.show_toast("Eliminated. Wait for match end or press ESC.", duration=2.0)
            else:
                self.ui_manager.show_toast("Eliminated. Respawning...", duration=1.5)
            return
    ...
    def get_objective_lines(self):
        ...
        phase = str(self.match_phase or "lobby").upper()
        timer_label = ""
        if self.match_phase == "countdown":
            timer_label = f"Start In: {max(0, int(math.ceil(self.match_countdown)))}s"
        elif self.match_phase == "live":
            remaining = max(0, int(self.match_duration - self.match_elapsed)) if self.match_duration > 0 else 0
            timer_label = f"Time Left: {remaining}s"
        elif self.match_phase == "ended":
            timer_label = "Match Ended"
        else:
            ready_count = sum(1 for ready in self.lobby_ready_players.values() if ready)
            timer_label = f"Ready: {ready_count}/{max(1, len(self._active_multiplayer_player_ids()))}"

        lines = [
            f"{self.match_settings.display_name} [{phase}]",
    ...
    def unregister_player(self, player_id: str) -> None:
        ...
        if pid in self.player_teams:
            del self.player_teams[pid]
        if pid in self.lobby_ready_players:
            del self.lobby_ready_players[pid]
        if pid in self.player_ping_ms:
            del self.player_ping_ms[pid]
        if pid in self.player_position_history:
            del self.player_position_history[pid]
    ...
    def _remove_remote_player_entity(self, player_id: str) -> None:
        ...
        if pid in self.remote_authority_samples:
            del self.remote_authority_samples[pid]
        if self.network_bridge.mode == self.network_bridge.MODE_SERVER and self._is_multiplayer_match():
            self._queue_lobby_state_packet(force=True)
    ...
    def _spawn_remote_player_from_state(self, remote_state: PlayerSyncState) -> None:
        ...
        self._record_player_position_history(remote_player_id, remote_player.world_position)
    ...
    def _handle_team_assignment_packet(self, payload: dict) -> None:
        ...
        if not player_id or not team_id:
            return
        self._register_player_for_match(player_id)
    ...
    def input(self, key: str) -> None:
        ...
        if self.state == "playing" and local_player:
            if key == "f1" and self._is_multiplayer_match():
                self._toggle_local_ready_state()
                return
            if key == "f2" and self._is_multiplayer_match() and self.network_bridge.mode == self.network_bridge.MODE_SERVER:
                if self.match_phase in ("lobby", "countdown") and self.mode_id == "duel_1v1":
                    mode = self.mode_registry.resolve_mode("duel_1v1")
                    pool = [map_id for map_id in mode.map_pool]
                    if pool:
                        current = str(self.current_map_id or pool[0])
                        try:
                            idx = pool.index(current)
                        except ValueError:
                            idx = -1
                        self.preferred_map_id = pool[(idx + 1) % len(pool)]
                        self.current_map_id = self.preferred_map_id
                        self.match_settings = self.mode_registry.build_match_settings(
                            mode_id=mode.mode_id,
                            map_id=self.current_map_id,
                            requested_team_size=1,
                            requested_max_players=2,
                        )
                        self._queue_match_config_packet()
                        self._queue_lobby_state_packet(force=True)
                        self.ui_manager.show_toast(f"Duel map set: {self.current_map_id}", duration=1.2)
                return
            if key == "f5":
                if self.match_settings and self.match_settings.is_multiplayer:
    ...
    def _pump_network(self) -> None:
        if not self.network_bridge.is_active:
            ...
            if (
                self.state == "playing"
                and self._is_multiplayer_match()
                and self.multiplayer_connection_mode == "client"
                and not self.disconnect_notice_active
            ):
                self.disconnect_notice_active = True
                self.ui_manager.show_toast("Disconnected from host. Returned to multiplayer menu.", duration=2.2)
                self.show_multiplayer_mode_select()
            return
        ...
        if self.network_bridge.mode == self.network_bridge.MODE_CLIENT:
            self.ping_timer -= self.frame_dt
            if self.ping_timer <= 0.0:
                self.ping_timer = self.ping_interval
                self._queue_ping_packet()

        local_player = self.get_local_player()
        if self.state == "playing" and local_player and local_player.alive:
            self._record_player_position_history(self.local_player_id, local_player.world_position)
            if not self.network_bridge.connection_ready:
    ...
    def _handle_network_packet(self, packet: dict) -> None:
            ...
            if isinstance(payload, dict):
                ...
                if player_id:
                    ...
                    if self.network_bridge.mode == self.network_bridge.MODE_SERVER:
                        self._queue_lobby_state_packet(force=True)
            return
        if packet_type == "lobby_ready":
            if self.network_bridge.mode != self.network_bridge.MODE_SERVER:
                return
            payload = packet.get("payload", {})
            if not isinstance(payload, dict):
                return
            declared_player_id = str(payload.get("player_id", "") or "")
            resolved_player_id = self._resolve_attacker_id_from_packet(packet)
            player_id = resolved_player_id if resolved_player_id else declared_player_id
            if not player_id:
                return
            self._set_player_ready_state(player_id, bool(payload.get("ready", False)), broadcast=True)
            return
        if packet_type == "match_config":
            self._handle_match_config_packet(packet.get("payload", {}))
            return
        if packet_type == "lobby_state":
            self._handle_lobby_state_packet(packet.get("payload", {}))
            return
        if packet_type == "match_start":
            ...
            self.match_elapsed = 0.0
            return
        if packet_type == "match_end":
            self.match_phase = "ended"
            return
        if packet_type == "team_assignment":
            self._handle_team_assignment_packet(packet.get("payload", {}))
            return
        if packet_type == "health_sync":
            self._handle_health_sync_packet(packet.get("payload", {}))
            return
        if packet_type == "ping":
            self._handle_ping_packet(packet.get("payload", {}))
            return
        if packet_type == "pong":
            self._handle_pong_packet(packet.get("payload", {}))
            return
        if packet_type == "ping_report":
            self._handle_ping_report_packet(packet.get("payload", {}))
            return
        if packet_type == "damage_request":
            self._handle_damage_request_packet(packet, packet.get("payload", {}))
            return
        if packet_type == "damage_event":
            self._handle_damage_event_packet(packet.get("payload", {}))
            return
        if packet_type == "kill_feed":
            self._handle_kill_feed_packet(packet.get("payload", {}))
            return
        if packet_type == "respawn":
            self._handle_respawn_packet(packet.get("payload", {}))
            return
        ...
        if remote_player_id not in self.remote_player_states and len(self.remote_player_states) >= 256:
            return
        self._register_player_for_match(remote_player_id)
        ...
        self.remote_last_seen_time[remote_player_id] = float(self.network_clock)
        self._record_player_position_history(remote_player_id, Vec3(remote_state.position.x, remote_state.position.y, remote_state.position.z))
    ...
    def _sanitize_remote_state(self, remote_state: Optional[PlayerSyncState]) -> Optional[PlayerSyncState]:
        ...
        if not self._validate_remote_movement(remote_state):
            return None
        return remote_state
