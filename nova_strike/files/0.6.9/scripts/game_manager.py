...
from ursina import Vec3, application, camera, color, destroy, mouse, raycast, render, scene, time, window
...
class GameManager:
    ...
    def _purge_stale_world_lights(self, keep_root_name: str = "") -> None:
        try:
            light_nodes = render.find_all_matches("**/+Light")
        except Exception:
            return
        try:
